...
съесть. Определи:
1. Из чего состоит блюдо — отдельными позициями (например: "куриная грудка
   гриль", "рис отварной", "салат из огурцов").
...
Ответь СТРОГО валидным JSON без markdown-обёртки и без пояснений:
{
  ...
  "items": [
    { "search_term": "string", "quantity_grams": number, "state": "raw"|"cooked"|"unknown" }
  ]
}`;
